...
gdf_error close_dmo_file(mvfs_connection *conn, mvfs_handle fd) {
    int rc = mvfs_close_mc(conn, fd);
    if (rc < 0) {
        perror("mvfs_close_mc failed. \n");
        exit(-3);
    }
    return GDF_SUCCESS;
}
...
gdf_error read_csv(csv_read_arg *args)
{
	...
	int fd = 0;
        ...
        mvfs_connection *connection_;
        mvfs_connect_option conn_opt_;
...
////////////////////###
                ...
                int rc = mvfs_connect_mc(&connection_, &conn_opt_);
                if (rc < 0) {
         //            DMO_ERROR("Connect to {} failed:{}", socket, rc);
                     perror("mvfs_connect_mc failed.\n");
                     exit(-1);
                }

                mvfs_handle fd = NULL;
                int status = mvfs_open_mc(connection_, &fd, args->filepath_or_buffer);
                if (status < 0) {
                    perror("mvfs_open_mc failed.\n");
                    exit(-2);
                }
                ...
                if (rc < 0) {
                    perror("mvfs_fgetattr_mc failed.\n");
                    close_dmo_file(connection_, fd);
                    exit(-4);
                }
                const auto file_size = attr.size;
...
/////////////////////////////***
                map_data = mvfs_mmap_mc(connection_, fd, map_offset, map_size);

                if (map_data == MAP_FAILED || map_size==0) { close_dmo_file(connection_, fd); checkError(GDF_C_ERROR, "Error mapping file"); }
/////////////////////////////***
	}
	...
	if (args->input_data_form == gdf_csv_input_form::FILE_PATH)
	{
		...
		//munmap(map_data, map_size);
                close_dmo_file(connection_ , fd);
                mvfs_munmap_mc(connection_, map_data, map_size);
	}
